...
export default class AccountsController {
    ...
    async update(req: Request , res: Response): Promise<void> { 
        ...
        if (nom) { account.set({nom: nom}) ; }
        if (prenom) { account.set({prenom: prenom}) ; }
        if (email) { account.set({email: email}) ; }
        if (mot_de_pass) { 
            const security: SecurityUtils = new SecurityUtils();
            const hash: string = await security.argon2Hash(mot_de_pass) ; 
            account.set({mot_de_pass: hash}) ; }
        if (a_badge) { account.set({a_badge: a_badge}) ; }
        if (est_admin) { account.set({est_admin: est_admin}) ; }
        if (est_employee) { account.set({est_employee: est_employee}) ; }
        if (id_post) { account.set({id_post: id_post}) ; }
