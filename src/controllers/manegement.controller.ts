...
export class ManegementController {
	...
	async monthStats(req: Request, res: Response): Promise<void> {
		...
		if (espace === undefined) {
			const startDate = new Date(`${currentYear}-${monthNumber}-01`);
			let endDate = new Date(`${currentYear}-${month - 1}-01`);

			if (month === 12) {
				endDate = new Date(`${currentYear}-${month}-31`);
			}
			...
			try {
				...
				const startDate = new Date(`${currentYear}-${monthNumber}-01`);
				const endDate = new Date(`${currentYear}-${parsedMonth + 1}-01`);
